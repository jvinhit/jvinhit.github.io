...
<button class="hover:bg-indigo-600 text-white shadow-lg">Sai</button>

<!-- ✅ lặp tiền tố cho từng thuộc tính cần đổi khi hover -->
<button class="hover:bg-indigo-600 hover:text-white hover:shadow-lg">Đúng</button>
```
...
| `focus:` | `:focus` | được focus (chuột *hoặc* bàn phím) | ring/outline |
| `focus-visible:` | `:focus-visible` | focus *qua bàn phím* (ẩn khi click chuột) | ring a11y "đúng lúc" |
...
```html
<input
  class="border border-slate-300 px-3 py-2 rounded-md
         disabled:opacity-50 disabled:pointer-events-none
         required:border-red-300
         user-invalid:border-pink-500"
/>
```
...
<ul class="divide-y divide-slate-200 rounded-lg border">
  <li class="p-3 first:rounded-t-lg last:rounded-b-lg odd:bg-slate-50">Row 1</li>
...
<label class="before:mr-1 before:text-red-500 before:content-['*']">Email</label>

<!-- placeholder nhạt + nghiêng, vùng bôi đen tím -->
<input class="placeholder:italic placeholder:text-slate-400 selection:bg-indigo-200" placeholder="you@example.com" />
...
> **Khoảng trắng trong arbitrary variant dùng `_` (gạch dưới).** Vì class không chứa được dấu cách, Tailwind quy ước `_` → space. Nên `[&_p]` nghĩa là `& p` (hậu duệ), còn `[&>p]` là `& > p` (con trực tiếp). Nếu *thật sự* cần một gạch dưới trong giá trị (hiếm), escape bằng `\_`.
...
- Với variant **cùng họ** có thứ tự ngữ nghĩa (ví dụ `group-*` lồng `has-*`), thứ tự *có* ý nghĩa — viết theo quan hệ bao ngoài.
...
<button class="bg-blue-500 hover:bg-blue-700 disabled:hover:bg-blue-500">Giữ màu khi disabled</button>
<div class="group">
