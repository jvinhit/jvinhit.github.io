...
- pnpm 10+, Yarn 4.14+ và Bun đều deny build script của dependency theo mặc định; npm vẫn cần `--ignore-scripts` hoặc config project.
...
# pnpm equivalent: dependency scripts are off by default; approve specific ones:
pnpm approve-builds
```
...
- npm/Yarn Classic có thể chạy lifecycle script của **mọi** dependency có định nghĩa; manager hiện đại đang chuyển sang deny-by-default.
