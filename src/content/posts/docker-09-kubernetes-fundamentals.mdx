...
description: 'Từ một host tới cluster: kiến trúc Kubernetes, Pod, Deployment, Service, kubectl, kind và lần deploy ứng dụng đầu tiên.'
pubDate: 2025-10-27
...
Đây là **Phần 9 của chặng nền tảng** về Docker → Compose → Kubernetes. **Docker Compose** chạy container trên **một máy** — bạn khai báo service trong YAML và `docker compose up` bật cả stack ([Phần 5](/blog/docker-05-compose-fundamentals/), [Phần 6](/blog/docker-06-compose-in-depth-profiles-healthchecks/)). Đủ cho dev local và nhiều triển khai nhỏ. Khi cần **nhiều máy**, **tự phục hồi**, rollout có kiểm soát và **scale ngang** trên nhiều node, bạn cần **orchestrator** — chuẩn công nghiệp là **Kubernetes** (**k8s**). Zero downtime còn phụ thuộc readiness, capacity, graceful shutdown và tính tương thích của ứng dụng; Kubernetes không tự bảo đảm chỉ vì dùng Deployment.
...
| `volumes:` | `emptyDir`, **PV** + **PVC** ([Phần 16](/blog/docker-16-kubernetes-stateful-storage/)) |
...
- Gọi thẳng IP Pod — IP **ngắn hạn**; luôn qua **Service**, và dùng Gateway/Ingress cho HTTP traffic từ ngoài cluster.
...
**[Phần 10 — Kubernetes thực hành & Debug](/blog/docker-10-kubernetes-in-practice-debugging/)** — ConfigMap và Secret, probe startup/readiness/liveness, resource request/limit, rollout và **checklist debug `kubectl`** khi Pod không lên. Networking nâng cao nằm ở [Phần 14](/blog/docker-14-kubernetes-networking-internals/); storage bền vững ở [Phần 16](/blog/docker-16-kubernetes-stateful-storage/).
