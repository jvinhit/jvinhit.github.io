...
> **Phần 9/10** trong series Docker → Compose → Kubernetes. Trước: Tiếp: Mỗi phần kết thúc bằng **bài tập**; hãy làm, đừng chỉ đọc.

**Docker Compose** chạy container trên **một máy** — bạn khai báo service trong YAML và `docker compose up` bật cả stack ([Phần 5](/blog/docker-05-compose-fundamentals/), [Phần 6](/blog/docker-06-compose-in-depth-profiles-healthchecks/)). Đủ cho dev local và nhiều triển khai nhỏ. Khi cần **nhiều máy**, **tự phục hồi**, **rolling update** không downtime và **scale ngang** trên nhiều node, bạn cần **orchestrator** — chuẩn công nghiệp là **Kubernetes** (**k8s**).

Thẳng thắn: Kubernetes **nặng** hơn Docker — nhiều khái niệm, YAML và bộ phận chuyển động. Bạn đúng chỗ vì đã hiểu **image, container, mạng, volume và Compose** từ Phần 1–8. Phần này giới thiệu **mô hình tư duy** và **cluster đầu tiên** trên laptop bằng **kind**. Phần 10 đi sâu config, probe và debug k8s.
...
- API REST; `kubectl` gọi vào đây
- Chọn node cho Pod mới
- Giữ Deployment/ReplicaSet khớp spec
- Lưu trạng thái cluster
- Chạy Pod trên node qua runtime
- Quy tắc mạng Service → Pod
