...
> **Phần 3/10** trong series Docker → Compose → Kubernetes. Trước: Tiếp:

Trong [Phần 1](/blog/docker-01-fundamentals-containers-images/) bạn đã biết mỗi container đang chạy có một **layer ghi-được** mỏng trên các layer image chỉ-đọc. Mọi thứ bạn tạo hoặc sửa ở đó — file database, upload, chỉnh config — **chết khi container bị xóa**. Ổ với web server stateless; thảm họa với Postgres.
...
**5.** Dừng và xóa container bài tập, rồi `docker volume prune` và xác nhận volume không dùng đã mất (hoặc tạo lại `ex-pgdata` nếu còn cần).
