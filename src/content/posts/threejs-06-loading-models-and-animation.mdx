...
`action.play()` tự nó không làm gì animate. Mixer cần **delta time** mỗi frame — và đây chính là lý do ta dùng `clock.getDelta()` từ Phần 1.
...
- **Animation nhanh lên trên màn hình tần số cao?** Bạn truyền bước cố định thay vì `clock.getDelta()`.
...
- **Nhiều model, một mixer?** Đừng — mỗi model có animation cần `AnimationMixer` riêng.
