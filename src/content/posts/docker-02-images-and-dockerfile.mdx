...
Đây là **Phần 2 của series 10 bài** về Docker → Compose → Kubernetes. Ở [Phần 1 — Container, Image & mô hình tư duy](/blog/docker-01-fundamentals-containers-images/) bạn đã học **image là khuôn mẫu chỉ-đọc** và **container là thực thể đang chạy**. Phần này là lúc bạn ngừng chỉ kéo `nginx` và `node` từ Docker Hub và **tự build image** bằng `Dockerfile`.

Mỗi phần kết thúc bằng **bài tập**; hãy làm trên máy thật. Bạn sẽ viết Dockerfile, tận dụng build cache, thu nhỏ image bằng multi-stage build và gắn tag image đúng cách.

---

## Giải phẫu Dockerfile — các chỉ thị lõi

Một **Dockerfile** là công thức văn bản: mỗi dòng là một chỉ thị trở thành một **layer image** khi bạn build. Một app tối giản có thể như sau:
...
> **`RUN` vs `CMD`**: `RUN` chạy khi `docker build`; `CMD` chạy khi `docker run`. Nhầm hai cái là lỗi kinh điển của người mới.
...
- **Image khổng lồ** — dùng `node:20` thay vì `node:20-alpine`, nhét devDependencies, không multi-stage.
...
**Phần 3** — volume, bind mount và biến môi trường để dữ liệu sống sót khi container restart.
