...
pubDate: 2026-09-19
tags: [performance, rendering, css, reflow, web_perf_12]
series: web-perf
seriesOrder: 17
draft: true
---
...
```text
Đổi transform / opacity        → chỉ COMPOSITE        ⚡ rẻ nhất (compositor lo)
Đổi color / background         → PAINT + composite    🙂 vừa
Đổi width / top / margin / DOM → LAYOUT + paint + comp 🐢 đắt nhất (reflow)
```

Layout (reflow) là bước đắt nhất vì đổi hình học một phần tử có thể buộc tính lại cả cây xung quanh. Mục tiêu: với animation và cập nhật thường xuyên, **chỉ chạm `transform`/`opacity`**.
...
```text
Thuộc tính gây forced reflow khi đọc (sau khi ghi):
...
`contain-intrinsic-size` quan trọng: thiếu nó, các section bị bỏ qua có chiều cao 0 → scrollbar nhảy và CLS. Cho kích thước placeholder hợp lý.
...
```css
/* Mỗi card là một "hộp" độc lập: đổi nội dung trong card không reflow cả trang */
.card {
...
Containment đặc biệt giá trị cho component lặp lại nhiều (list item, card grid): nó giới hạn "bán kính ảnh hưởng" của mỗi cập nhật.

---

## 5. will-change — dùng đúng liều

`will-change` báo trước cho trình duyệt thăng một phần tử lên layer riêng để chuẩn bị animate (transform/opacity rẻ). Nhưng **lạm dụng phản tác dụng**: mỗi layer tốn bộ nhớ GPU.
...
✅ Lành mạnh:
  ...
  - dùng CSS thay JS cho animation khi có thể (chạy off-main-thread)
⚠️ Coi chừng:
  - animate width/height/top/left (gây reflow mỗi khung)
  - box-shadow/filter lớn (paint đắt) animate liên tục
  - DOM khổng lồ (>vài nghìn node) làm mọi recalculation chậm
```
...
- Thay đổi style có ba mức chi phí: **transform/opacity (composite, rẻ)** < color/bg (paint) < **width/top/DOM (layout/reflow, đắt)**; animate thì chỉ chạm transform/opacity.
...
- **`will-change`** dùng đúng liều (bật trước animate, gỡ sau); lạm dụng gây layer explosion tốn VRAM.
