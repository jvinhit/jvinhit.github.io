...
Open the full demo \{Mở demo đầy đủ\}: [/tools/svg-motion-path-demo/](/tools/svg-motion-path-demo/).
...
| `auto` | rotates so local +x faces the tangent — banks into turns \{xoay để +x hướng tiếp tuyến — nghiêng vào cua\} |
| `auto-reverse` | like auto, but facing backward along the path \{như auto, nhưng quay ngược dọc path\} |
...
- **`keyPoints` needs `keyTimes` and `calcMode`.** Use them together or the pacing is ignored. \{**`keyPoints` cần `keyTimes` và `calcMode`.**\}
- **For heavy or interactive motion, prefer `offset-path`/JS.** Many simultaneous `animateMotion`s can get expensive (Part 19). \{**Chuyển động nặng/tương tác thì ưu tiên `offset-path`/JS.**\}
