...
description: 'Hiểu đúng container, image, layer và vòng đời docker run; so sánh với VM, thực hành container đầu tiên và các lệnh cốt lõi.'
pubDate: 2025-09-01
...
- `stop` gửi tín hiệu để tiến trình kết thúc có kiểm soát; `pause` mới là đóng băng tạm thời. `rm` xóa container. `docker ps -a` cho thấy container đã dừng vẫn chiếm chỗ.
...
**[Phần 2 — Image & Dockerfile](/blog/docker-02-images-and-dockerfile/)**: bạn sẽ ngừng dùng image của người khác và **tự build image của mình** — layer, build cache, multi-stage build và `.dockerignore`.
