...
> **Mental model**: a chat prompt is a *question*; an agent prompt is an *API contract*. \{Mental model: prompt chat là một *câu hỏi*; prompt agent là một *hợp đồng API*.\} You are specifying inputs, output shape, and failure behavior for code that runs unattended hundreds of times. \{Bạn đang đặc tả input, hình dạng output, và hành vi khi lỗi cho đoạn code chạy không người giám sát hàng trăm lần.\} Sloppy wording that a human would forgive becomes a production bug at scale. \{Một câu chữ cẩu thả mà con người bỏ qua sẽ thành bug production khi nhân lên quy mô lớn.\}
