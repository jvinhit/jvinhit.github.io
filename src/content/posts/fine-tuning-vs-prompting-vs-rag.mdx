...
> **Mental model \{Mô hình tư duy\}:** You've just hired a brilliant engineer — the base model \{Bạn vừa thuê một kỹ sư giỏi — base model\}. **Prompting** is the instructions you hand them per task \{**Prompting** là chỉ dẫn bạn đưa cho từng task\}. **RAG** is giving them the company wiki to look things up before answering \{**RAG** là đưa họ wiki công ty để tra cứu trước khi trả lời\}. **Fine-tuning** is sending them on a months-long course that rewires their instincts \{**Fine-tuning** là gửi họ đi khoá học dài hạn để đổi cả bản năng\}. You always reach for the cheapest lever that closes the gap — and you almost never start with the training course \{Bạn luôn chọn lever rẻ nhất đủ lấp gap — và gần như không bao giờ bắt đầu bằng khoá đào tạo\}.
...
- **Fine-tuning to add facts \{Fine-tune để thêm fact\}** — knowledge that changes belongs in RAG, not frozen weights \{knowledge hay đổi thuộc về RAG, không phải weight đông cứng\}.
- **Skipping the eval baseline \{Bỏ baseline eval\}** — you can't prove a fine-tune helped if you never measured prompt + RAG first \{không thể chứng minh fine-tune có ích nếu chưa đo prompt + RAG trước\}.
- **Tiny, dirty datasets \{Dataset nhỏ, bẩn\}** — under ~100 inconsistent pairs teach the model noise, not behavior \{dưới ~100 cặp không nhất quán dạy model noise, không phải behavior\}.
- **Confusing knowledge with behavior \{Nhầm knowledge với behavior\}** — the single root cause of most wasted GPU hours \{gốc rễ duy nhất của hầu hết giờ GPU lãng phí\}.
- **Fine-tuning on the eval set \{Fine-tune trên eval set\}** — inflated offline scores, production regressions \{điểm offline ảo, regress production\}.
- **Forgetting the regression slice \{Quên lát regression\}** — specialize on one task, silently break reasoning or safety elsewhere \{chuyên một task, âm thầm hỏng reasoning hoặc safety chỗ khác\}.
...
**When NOT to fine-tune \{Khi KHÔNG fine-tune\}**: you haven't exhausted prompt + RAG on a real eval, you have under 100 reliable examples, or requirements change weekly \{chưa cạn prompt + RAG trên eval thật, có dưới 100 ví dụ tin cậy, hoặc requirement đổi hàng tuần\}.

## Quick Start Guide \{Bắt đầu nhanh\}

Walk the decision in order — stop at the first lever that hits your eval bar \{Đi theo thứ tự — dừng ở lever đầu tiên đạt ngưỡng eval\}:

1. **Classify the gap \{Phân loại gap\}** — knowledge, behavior, or both? \{knowledge, behavior, hay cả hai?\}
2. **Build a frozen eval set first \{Dựng eval set cố định trước\}** — you cannot compare levers without it (→ Part 7) \{không so sánh được lever nếu thiếu nó (→ Phần 7)\}.
3. **Try a strong prompt + few-shot \{Thử prompt mạnh + few-shot\}** — if it clears the bar, stop here \{nếu đạt ngưỡng, dừng tại đây\}.
4. **Add RAG for any knowledge gap \{Thêm RAG cho mọi gap knowledge\}** — chunking, hybrid search, citations (→ [RAG Guide](/blog/rag-retrieval-augmented-generation-guide/)) \{chunking, hybrid search, citation\}.
5. **Only then consider LoRA/SFT \{Chỉ sau đó cân nhắc LoRA/SFT\}** — when eval proves a behavior ceiling and you have clean labeled data (→ [Fine-tuning Basics](/blog/fine-tuning-llm-basics/)) \{khi eval chứng minh trần behavior và bạn có data labeled sạch\}.
