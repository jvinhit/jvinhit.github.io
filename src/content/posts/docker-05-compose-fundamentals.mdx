...
Chỉ publish **`ports`** cho service host cần gọi; DB thường nội bộ (`db:5432` trên mạng project). Compose đọc **`.env`** cho `${VAR}` — không commit secret ([Phần 3](/blog/docker-03-data-volumes-bind-mounts-env/)).
