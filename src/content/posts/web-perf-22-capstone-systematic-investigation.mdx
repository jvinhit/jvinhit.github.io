...
description: 'Ghép mọi phần chuyên sâu thành một khung chẩn đoán: triệu chứng → metric → công cụ → cách sửa. Điều tra một app chậm thật từ field data tới root cause across mọi tầng.'
pubDate: 2026-09-29
tags: [performance, investigation, debugging, web-vitals, web_perf_12]
series: web-perf
seriesOrder: 22
draft: true
---
...
Nguyên tắc xuyên suốt cả series: **đo trước, sửa sau**; **field là chân lý, lab để chẩn đoán**; **sửa một thứ một lúc** để biết cái gì có tác dụng.
...
LCP cao (nội dung chính hiện chậm)
  Đo:    LCP Attribution → TTFB? load delay? load time? render delay?
  Công cụ: WebPageTest waterfall, Lighthouse, RUM Attribution
  Sửa:   TTFB → SSR/edge/cache (Part 15,19); load → preload + fetchpriority ảnh LCP (Part 14);
...
INP cao (bấm/gõ phản hồi chậm)
  ...
  Công cụ: LoAF API (script gây chậm), Performance panel
...
TTFB cao (server chậm trả byte đầu)
  Công cụ: server timing, WebPageTest
...
DevTools cài đặt tái hiện:
  - CPU throttling 4x–6x (mô phỏng điện thoại tầm trung)
  - Network: Slow 4G
...
Nếu field nói "mobile 4G p75 INP = 600ms" mà lab không tái hiện, gần như chắc bạn chưa throttle đủ. Reproduce đúng là một nửa của việc sửa.
...
③ TÁI HIỆN:   CPU 4x + Slow 4G → click filter mất ~500ms
...
⑦ CHỐT:       thêm budget INP cho route này + LoAF alert (Part 21)
```
...
❌ Đo trên máy dev mạnh → bỏ sót lỗi của người dùng thật
...
- Điều tra = **truy sâu một triệu chứng** tới root cause; vòng lặp: đo (field) → phân rã (Attribution) → tái hiện (lab) → tìm gốc (đúng công cụ) → sửa một thứ → đo lại → chốt budget.
...
- **Tái hiện đúng điều kiện** (CPU 4x, Slow 4G, đúng thiết bị, cold/warm) — lỗi field hiếm khi hiện trên máy dev mạnh.
