...
> **Mental model \{Mô hình tư duy\}:** Shipping an agent without eval is like refactoring a large codebase with no tests — it compiles, the demo works on your machine, and you have no idea what you just broke for everyone else \{Ship agent mà không có eval giống như refactor codebase lớn mà không có test — vẫn compile, demo chạy trên máy bạn, và bạn không biết vừa làm hỏng gì cho người khác\}. Eval is the **test suite for non-deterministic code** \{Eval là **test suite cho code không deterministic**\}.
...
- **"It works on three prompts" \{"Chạy ổn trên ba prompt"\}** — demo-driven confidence with no golden set behind it \{tự tin theo demo, không có golden set đứng sau\}.
- **Trusting LLM-as-judge blindly \{Tin LLM-as-judge mù quáng\}** — never calibrated against human labels; position and verbosity bias left unchecked \{chưa calibrate với nhãn human; bias vị trí và độ dài chưa kiểm soát\}.
- **Aggregate-only reporting \{Chỉ báo số tổng\}** — a 95% average quietly hides a 60% tool-call stratum \{trung bình 95% lặng lẽ che stratum tool-call 60%\}.
- **Final-answer-only scoring for agents \{Chỉ chấm câu trả lời cuối với agent\}** — misses wasteful or unsafe trajectories that happen to land on the right answer \{bỏ sót trajectory lãng phí hoặc thiếu an toàn mà tình cờ ra đúng\}.
- **An eval set that never grows \{Eval set không bao giờ lớn lên\}** — production failures don't flow back as new cases, so the same bug ships twice \{failure production không quay lại thành case mới, nên cùng một bug ship hai lần\}.
- **Training or judging on the eval set \{Train hoặc judge trên eval set\}** — inflated offline scores, real-world collapse \{điểm offline ảo, sập ngoài thực tế\}.

## When Should You Use This? \{Khi nào nên dùng\}

Match eval investment to blast radius \{Khớp đầu tư eval với mức độ ảnh hưởng\}:
...
**When to skip the heavy machinery \{Khi nào bỏ qua bộ máy nặng\}**: a one-off script or a demo you'll throw away tomorrow \{một script chạy một lần hoặc demo bỏ đi ngày mai\}. **The moment real users depend on the output, eval stops being optional** \{Ngay khi user thật phụ thuộc vào output, eval hết là tuỳ chọn\}.

## Quick Start Guide \{Bắt đầu nhanh\}

Bootstrap an eval harness in an afternoon \{Dựng eval harness trong một buổi chiều\}:

1. **Collect 20–50 real cases \{Thu 20–50 case thật\}** — pull from logs or past bug reports; redact PII \{lấy từ log hoặc bug report cũ; redact PII\}.
2. **Write `golden_set.json` \{Viết `golden_set.json`\}** — input, reference (or acceptance criteria), and a `category` tag per case \{input, reference (hoặc tiêu chí chấp nhận), và tag `category` mỗi case\}.
3. **Pick one metric that matches the task \{Chọn một metric khớp task\}** — task success for agents, groundedness for RAG, exact match for classification \{task success cho agent, groundedness cho RAG, exact match cho classification\}.
4. **Add an LLM-as-judge with a rubric \{Thêm LLM-as-judge kèm rubric\}** — then sanity-check 10 of its scores against your own \{rồi đối chiếu 10 điểm của nó với điểm bạn tự chấm\}.
5. **Wire it into CI \{Gắn vào CI\}** — fail the PR if the aggregate drops past a tolerance or any P0 case breaks \{fail PR nếu số tổng tụt quá ngưỡng hoặc một P0 case vỡ\}.

> Then close the loop \{Rồi khép vòng lặp\}: every production failure becomes a new golden case, so the same bug can never ship twice \{mỗi failure production thành một golden case mới, để cùng một bug không bao giờ ship hai lần\}.
