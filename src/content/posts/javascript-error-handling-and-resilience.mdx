...
- `throw` đồng bộ trong `try`
- Lỗi từ gọi hàm đồng bộ trong `try`
- Promise reject khi `await` trong `async` `try`
...
> Mô hình tư duy: `try/catch` là **ranh giới stack-frame**, không phải **ranh giới thời gian**. Mọi thứ lên lịch sau cần handler riêng hoặc lưới an toàn global.
...
> Đừng chỉ dựa global. Là **lưới**, không phải **chiến lược**. Mọi entry point `async` vẫn cần xử lý local.
...
1. **Throw instance `Error`**, không string — giữ `stack`, `name`, `cause`.
2. **`try/catch` là đồng bộ** — callback timer và Promise reject trần cần handler riêng.
3. **Custom error + `cause`** giúp app nhiều lớp debug được.
4. **Handler global** là lưới an toàn — không phải chiến lược chính.
5. **Resilience = degrade + retry + fallback**, với failure hiện rõ.
6. **Báo cáo có context** và **không nuốt lỗi**.
