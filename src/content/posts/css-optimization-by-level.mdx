...
| Chi phí | Thuộc tính | Kích hoạt |
|---|---|---|
| Rất tốn | `width`, `height`, `top`, `left`, `margin`, `padding`, `font-size` | Layout + Paint + Composite |
| Trung bình | `color`, `background-color`, `box-shadow`, `border-color` | Paint + Composite |
| Rẻ | `transform`, `opacity`, `filter` | Composite only |

Quy tắc vàng: animate CHỈ `transform` và `opacity`. Mọi thứ khác buộc browser tính lại layout hoặc repaint.
...
| Cách tiếp cận | Kích thước bundle | Chi phí runtime | Bảo trì |
|---|---|---|---|
| Utility-first (Tailwind) | Tăng logarithm | Zero (static classes) | Dễ xoá |
| CSS Modules | Theo component | Zero (scoped) | Medium |
| CSS-in-JS (runtime) | Small initial | Thực thi JS mỗi render | Coupled to JS |
| CSS-in-JS (zero-runtime) | Per-component | Zero (extracted at build) | Medium |
| Global stylesheet | Tăng tuyến tính | Zero | Khó xoá |
...
| Font total | ≤ 100KB | Tránh layout shift |
| Animations | compositor-only | 60Đảm bảo 60fps |
...
| Cấp | Tập trung | Tác động |
|---|---|---|
| **Junior** | Clean selectors, avoid layout thrashing, animate transform/opacity only | Ngăn regression |
| **Senior** | Critical CSS, containment, content-visibility, font optimization, unused CSS removal | Cải thiện CWV đo được |
| **Principal** | CSS architecture, design tokens, performance budgets, layer strategy, loading patterns, continuous monitoring | Mở rộng performance toàn tổ chức |
...
- Chỉ animate `transform` và `opacity`. Mọi thứ khác đều tốn kém.
