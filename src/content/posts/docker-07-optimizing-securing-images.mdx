...
> **Phần 7/10** trong series Docker → Compose → Kubernetes. Trước: Tiếp:

Ở [Phần 2](/blog/docker-02-images-and-dockerfile/) bạn đã học **build** image — layer, cache, multi-stage, `.dockerignore`. Ở [Phần 6](/blog/docker-06-compose-in-depth-profiles-healthchecks/) bạn điều phối stack nhiều service với profile và health check. Image **chạy được** chưa phải image **tốt**: Layer phình làm mọi pull và deploy chậm, mặc định root mở rộng vùng ảnh hưởng, secret nhúng vào layer sống mãi trong `docker history`. Phần này giúp image **nhỏ, nhanh và an toàn**.
...
|---|---|---|
| **Full** | `node:20`, `python:3.12` | Dễ debug; bề mặt tấn công lớn nhất |
| **Slim / Alpine** | `node:20-alpine`, `python:3.12-slim` | Nhỏ hơn nhiều; Alpine dùng `musl` — test dependency native |
| **Distroless** | `gcr.io/distroless/nodejs20-debian12` | Không shell, OS tối thiểu — tốt cho runtime production |
...
4. **Ghim base image** — `node:20-alpine` ổn định hơn `node:latest` cho cache.
...
**Lúc chạy**, đưa secret qua orchestrator hoặc env — không rebuild image với `ENV PASSWORD=...`.
...
**Fat**: `node:20` + copy hết trước; **slim**: alpine, dependency trước source, `USER node` (giống multi-stage Phần 2).
...
- **`.dockerignore` + quét + tag ghim** giữ build nhanh và CVE lộ trước khi deploy.

---

## Tiếp theo

**Phần 8** — `docker logs`, `exec`, inspect, exit code, bẫy mạng/volume, và checklist debug khi container hành xử lạ.
