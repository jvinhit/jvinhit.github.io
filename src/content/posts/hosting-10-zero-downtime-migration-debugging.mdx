...
```bash
dig example.com A @<authoritative-nameserver> +noall +answer
...
| Static asset   | `curl -I https://example.com/assets/<hash>.js`                     | 200, correct content-type/cache header      |
...
2. Query trực tiếp từng nameserver mới: `dig @<new-ns> example.com A/MX/TXT`.
...
| Authoritative A/AAAA | `dig @<ns> example.com A`; `AAAA`                           | Nguồn sự thật có record đúng không                     |
| Recursive cache      | `dig @1.1.1.1`; `dig @8.8.8.8`                              | Resolver đang cache answer/TTL nào                     |
