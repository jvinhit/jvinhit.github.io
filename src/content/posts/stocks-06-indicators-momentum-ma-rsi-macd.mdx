...
Đây là **Phần 6 của series 22 phần**. Phần 5 nói về **mẫu hình** biểu đồ và nến (các *hình dạng* của giá). Phần này nói về **chỉ báo** — toán đắp lên giá để đo xu hướng, động lượng, và biến động.
...
### Làm mượt Wilder — vì sao con số thực hơi khác

Ví dụ trên dùng **trung bình đơn giản** của gain/loss để bạn theo từng bước bằng tay. RSI(14) thật trên app biểu đồ dùng **làm mượt Wilder** — một dạng trung bình hàm mũ với hệ số `1/N`. Giá trị RSI cuối cùng **xấp xỉ** giống nhau, nhưng các con số trung gian (average gain, average loss) sẽ khác, và càng chạy nhiều phiên thì càng lệch khỏi trung bình đơn giản.

Cách Wilder tính:

```text
  First Avg Gain = simple average of the first N gains
  First Avg Loss = simple average of the first N losses

  Then each new day rolls forward (Wilder smoothing):
    Avg Gain = ((Prev Avg Gain × (N − 1)) + Current Gain) / N
    Avg Loss = ((Prev Avg Loss × (N − 1)) + Current Loss) / N
```

Vì mỗi ngày mới chỉ chiếm trọng số `1/N`, một nhịp tăng/giảm đơn lẻ **không** làm RSI giật mạnh như trung bình đơn giản — đó là chủ đích của Wilder: làm mượt để giảm nhiễu. Bạn không cần tính tay; chỉ cần hiểu rằng RSI là một **trung bình có trí nhớ**, không phải ảnh chụp 14 ngày tách rời.
...
Giờ làm mượt đường MACD bằng **EMA 9 kỳ** để có **đường tín hiệu** (signal). Đây là nguồn gốc của các con số Signal — chúng không rơi từ trên trời, mà là EMA của chính đường MACD. Hệ số làm mượt:

```text
  k = 2 / (9 + 1) = 0.2     →  (1 − k) = 0.8

  EMA_today = (MACD_today × 0.2) + (Signal_yesterday × 0.8)

  Seed (Week 1): Signal = first MACD value = +1.0
                 (giống cách EMA tự seed bằng giá trị đầu)

  Week 2: Signal = (1.3 × 0.2) + (1.0   × 0.8) = 0.26 + 0.800 = 1.060
  Week 3: Signal = (1.5 × 0.2) + (1.060 × 0.8) = 0.30 + 0.848 = 1.148
  Week 4: Signal = (1.4 × 0.2) + (1.148 × 0.8) = 0.28 + 0.918 = 1.198
```

Giờ histogram = MACD − Signal, từng tuần (làm tròn 2 chữ số để dễ nhìn):

```text
  Week        MACD    Signal   Histogram (MACD − Signal)
  ────        ────    ──────   ─────────────────────────
  Week 2      +1.3    +1.06        +0.24
  Week 3      +1.5    +1.15        +0.35   ← peak (động lượng mạnh nhất)
  Week 4      +1.4    +1.20        +0.20   ← shrinking (đang đuối)
```

**Histogram co** từ 0.35 → 0.20 dù MACD vẫn dương — xu hướng tăng đang mất hơi *trước khi* MACD thực sự cắt xuống. Nếu tuần 5 MACD rớt dưới signal (histogram âm), đó là đảo chiều động lượng giảm — nhưng chỉ có ý nghĩa khi hành động giá và xu hướng lớn đồng thuận.
...
Độ lệch chuẩn (σ) đo giá dao động *rộng* bao nhiêu quanh trung bình. Cách tính: lấy mỗi giá đóng trừ SMA để ra **độ lệch**, bình phương từng độ lệch, lấy **trung bình các bình phương** đó (gọi là **phương sai**), rồi σ là **căn bậc hai** của phương sai. Phương sai lớn = giá bung rộng = dải Bollinger nở ra; phương sai nhỏ = giá nén lại = dải bóp vào.
...
Một cổ phiếu biến động đơn giản là *cần* stop rộng hơn — và đây chính là cây cầu nối sang **Phần 8**. ATR cho bạn một khoảng cách stop *theo biến động*, rồi khoảng cách đó nuôi thẳng vào công thức định cỡ vị thế:

```text
  Entry = 50,000,  ATR = 3,500
  Stop  = Entry − 2 × ATR = 50,000 − 7,000 = 43,000
  → Risk per share = 50,000 − 43,000 = 7,000 VND

  Part 8 dùng tiếp:
  Shares = (Capital × Risk%) ÷ 7,000

  Cùng 1% rủi ro: ATR càng lớn → stop càng rộng → số CP càng ít.
```

Nói cách khác chuỗi nhân quả là: ATR (biến động) → đặt stop = entry − 2×ATR → định cỡ vị thế. Stop rộng buộc **vị thế nhỏ hơn** cho cùng mức rủi ro — toàn bộ chuỗi đó là nội dung **Phần 8 (quản trị rủi ro & định cỡ vị thế)**.
...
## Cảnh báo: chỉ báo không phải quả cầu tiên tri

Trước khi rời phần này, hãy đóng đinh điều quan trọng nhất — **không một chỉ báo nào dự đoán tương lai**. Tất cả đều là toán đắp lên giá *quá khứ*. RSI 30 không "biết" giá sắp hồi; MACD cắt lên không "biết" xu hướng sắp đảo. Chúng chỉ tóm tắt những gì đã xảy ra theo cách dễ nhìn hơn.

Cái bẫy nguy hiểm nhất là **tự tin giả do chồng chỉ báo**. Vì mọi chỉ báo bắt nguồn từ cùng một giá, khi bạn mở RSI + MACD + Stochastic + ba đường MA, chúng phần lớn nói **cùng một điều** bằng vài giọng khác nhau. Bốn chỉ báo "đồng thuận" không phải bốn bằng chứng độc lập — nó là **một** bằng chứng được đếm bốn lần. Cảm giác chắc chắn tăng lên, nhưng xác suất thắng thì không.

> Bẫy: nhiều chỉ báo cùng sáng đèn xanh tạo cảm giác "kèo chắc". Thực ra bạn chỉ đang nghe một tín hiệu vọng lại nhiều lần. Tự tin không phải xác suất.

Thay vì chồng chỉ báo cùng gốc, hãy dùng một **confluence checklist** gọn — vài điều kiện *khác loại* (xu hướng, động lượng, khối lượng) phải cùng đồng thuận thì mới cân nhắc:

```text
  CONFLUENCE CHECKLIST (rút gọn) — chỉ cân nhắc khi PHẦN LỚN ô được tích:

    [ ] XU HƯỚNG     giá trên MA200 đang dốc lên (chế độ tăng)
    [ ] ĐỘNG LƯỢNG   RSI KHÔNG ở cực trị (không trên 80, không dưới 20)
    [ ] MACD         đường MACD mở rộng trên signal (histogram dương, đang nở)
    [ ] KHỐI LƯỢNG   phiên xác nhận có Rel Vol trên 1.5× trung bình

    0–1 ô  → bỏ qua
    2 ô    → yếu, chờ thêm
    3–4 ô  → tín hiệu đáng cân nhắc (vẫn phải có kế hoạch rủi ro Phần 8)

  Checklist KHÔNG đảm bảo thắng — nó chỉ lọc bớt những cược tệ nhất.
```

Bốn dòng này cố tình đo **bốn thứ khác nhau** (xu hướng, động lượng, sự mở rộng động lượng, và sự tham gia của đám đông), nên chúng *bổ sung* cho nhau thay vì lặp lại. Đó chính là khác biệt giữa confluence thật và tự tin giả.

Tóm lại: chỉ báo mô tả quá khứ, không dự đoán tương lai; chồng nhiều chỉ báo cùng gốc là tự tin giả — dùng một checklist gọn gồm các tín hiệu *khác loại*, và luôn để kế hoạch rủi ro quyết định cuối cùng.
