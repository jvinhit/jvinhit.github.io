...
Đây là **Phần 6 của series 10 bài** đưa bạn từ "mới nghe nói về Docker" đến tự tin build, chạy và *debug* ứng dụng container — Docker, Docker Compose và Kubernetes. Trước: Tiếp: Mỗi phần kết thúc bằng **bài tập**; hãy làm, đừng chỉ đọc.

Trong [Phần 5](/blog/docker-05-compose-fundamentals/) bạn đã khai báo stack nhiều dịch vụ trong `compose.yaml` và bật bằng `docker compose up`. Điều đó chỉ làm container **khởi động** — "đã start" không đồng nghĩa **sẵn sàng**. Phần này làm file Compose **vững và linh hoạt**: nội suy env, phụ thuộc theo health, profile tùy chọn, override file, scale và chính sách restart.
...
Pattern: healthcheck, service_healthy, profile, restart, nội suy biến.
